...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_pause = (ImageButton)findViewById(R.id.btn_stop);//这些必须在oncreate方法里面才可以!!!!
...
//                    ((ImageButton)view).setImageDrawable(getResources().getDrawable(R.drawable.play));//更换图标
                ...
                else
                {
                    mediaPlayer.start();
                    ispause=false;
//                    ((ImageButton)view).setImageDrawable(getResources().getDrawable(R.drawable.pause));//更换图标
                    ((ImageButton)view).setBackgroundResource(R.drawable.pause);
                }
            }
        });
...
//                int curt_seekBar = seekBar.getProgress();
//                int curt_time = (int)(curt_seekBar/(double)seekBar.getMax()*mediaPlayer.getDuration());
//                String current_time = MusicUtils.formatTime(curt_time);
//                current.setText(current_time);
            ...
            public void onStopTrackingTouch(SeekBar seekBar) {
                //int music_lenth =mediaPlayer.getDuration()/1000 ;
...
/*
        int no_need_to_add_to_vector=0;

        if(position == -1) //position为-1表示是 系统决定下一首播放的曲目
        {
            if(current_position_in_vector<V.size()-1) //当前播放的曲目处于vector数组内
            {

                position = V.get(current_position_in_vector);

                //current_position_in_vector++;//移向vector中的下一首歌曲

                no_need_to_add_to_vector = 1; //不需要加入数组
            }
                ...
                switch (PLAY_MODE) {
                    case 1:
                        ...
                        break;
                    case 2:
                        position = (int) (Math.random() * list.size());//随机播放
                        break;
                    case 3:
                        x = V.get(current_position_in_vector); //获得上一首播放的曲目
                        position = x; //单曲循环
        ...
        if(no_need_to_add_to_vector == 0) //需要加入数组
        {
            V.add(position); //把曲目加入V
            current_position_in_vector++;
        }
